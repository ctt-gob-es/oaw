...
public final class CSSUtils {
	...
	public static List<CSSProblem> evaluate(final Element rootElement, final CheckCode checkCode,
			final List<CSSResource> cssResources) {
		try {
	...
	private static org.dom4j.Document convertToDom4j(final Element element) {
		...
		if (element != null) {
			try {
				...
				Logger.putLog("No se ha podido convertir el documento a dom4j", CSSUtils.class,
	...
	private static String cleanHTML(final String html) {
		...
		try {
			...
			Logger.putLog("No se ha podido configurar el parseador de xml de tagsoup", CSSUtils.class,
					Logger.LOG_LEVEL_WARNING, e);
		}
	...
	private static String convertDocumentToString(final Element element) {
		try {
			...
			Logger.putLog("No se ha podido convertir el document a cadena", CSSUtils.class, Logger.LOG_LEVEL_WARNING,
					e);
		}
		return "";
	}
	...
	private static CSSAnalyzer getCSSAnalyzer(final Node node, final CheckCode checkCode)
		...
		case CheckFunctionConstants.FUNCTION_CSS_GENERATED_CONTENT:
			return new CSSGeneratedContentDocumentHandler(
					!checkCode.getFunctionNumber().isEmpty() ? Integer.parseInt(checkCode.getFunctionNumber()) : 1);
		...
		case CheckFunctionConstants.FUNCTION_FORCE_SPACING_CSS:
			return new CSSForceSpacingDocumentHandler();
		case CheckFunctionConstants.FUNCTION_HAS_RESPONSIVE_TAGS:
			return new CSSResponsiveElementsDocumentHandler();
		case CheckFunctionConstants.FUNCTION_FORCE_TRANSFORM:	
			return new CSSTransformDocumentHandler();	
		default:
			Logger.putLog("Warning: unknown function id: " + checkCode.getFunctionId(), CSSUtils.class,
	 ...
	 */
	public static Problem getProblemFromCSS(final CSSProblem cssProblem, final Check check,
			final Evaluation evaluation) {
		final PropertiesManager properties = new PropertiesManager();
		final SimpleDateFormat format = new SimpleDateFormat(
				properties.getValue("intav.properties", "complet.date.format.ymd"));
		...
		// Diferenciar si hay elemento asignado en CSSProblem y entonces usamos el
		// elemento en caso contrario crear un elemento ficticio
