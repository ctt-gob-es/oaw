...
public final class AccesibilityDeclarationCheckUtils {
	...
	/**
	 * Instantiates a new accesibility declaration check utils.
	 */
	...
	public static List<Element> getSectionLink(final NodeList links, final String sectionRegExp) {
		...
		for (int i = 0; i < links.getLength(); i++) {
			...
			if (link.hasAttribute("href") && !link.getAttribute("href").toLowerCase().startsWith("javascript") && !link.getAttribute("href").toLowerCase().startsWith("mailto")
					...
					&& !link.getAttribute("href").toLowerCase().endsWith(".doc") && !link.getAttribute("href").toLowerCase().endsWith(".epub")) {
				...
				try {
					...
					if (link.hasAttribute("title") && StringUtils.textMatchs(link.getAttribute("title").trim(), sectionRegExp) && includedLinks.add(href) && !checkIsPDFContent(link)) {
						linksFound.add(link);
					}
	...
	/**
	 * Comprueba que un link no devuelve un PDF para evitar links que puedan ser considerados enlaces de accesbilidad pero que sean otros elementos.
	 *
	 * @param link the link
	...
	/**
	 * Complemento del método {@link #hasContact(Document, String, String)} introducido en la revisión de la metodoloǵia UNE-2012.
	 * 
	 * Se separa del método anteriormente mencionado para generar un check adicional y permitir la convivencia de la UNE-2012 original y la revisión.
	 * 
	 * Se evalua si existen formularios de contacto en la propia página.
	 *
	 * @param document      documento HTML en formato DOM a analizar
	 * @param contactRegExp expresión regular para detectar enlaces a sección de contacto (contactar,contacte,...) correo directamente en el contenido (contacto@portal.es)
	 * @return true si se ha detectado una forma de contacto, false en caso contrario
	 */
	...
	public static boolean hasConformanceLevel(final Document document) {
		/*
		 * “Nivel .* A”, “Nivel .* AA”, “Nivel .* AAA” (.* por si se incluye algún texto adicional como “Nivel de Accesibilidad AA”, “Nivel de Conformidad AA”, etc.).--> Un texto con los patrones
		 * “doble A”, “triple AAA”, “prioridad X” (con x = 1, 2 o 3). Iconos de conformidad del W3C identificándolos buscando patrones similares a los anteriores en su texto alternativo o, en caso de
		 * ser enlaces, reconociendo las URLs de las páginas de conformidad del W3C.
		 */
	 ...
	 * @param sectionRegExp the section reg exp
	 * @return true, if successful
	 */
	public static boolean hasSection(final Document document, final String sectionRegExp) {
		final List<String> contactTexts = Arrays.asList(sectionRegExp.split("\\|"));
		// Encabezados en esta página
		final List<Element> headings = EvaluatorUtils.getHeadings(document);
		if (headings != null && !headings.isEmpty()) {
			for (Element heading : headings) {
	 ...
	 * @return the section
	 */
	public static List<Element> getSection(final Document document, final String sectionRegExp) {
		final List<String> contactTexts = Arrays.asList(sectionRegExp.split("\\|"));
		// Encabezados en esta página
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			final String nodeName = tagElement.getNodeName();
			if (nodeName.equalsIgnoreCase("h1") || nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h3") || nodeName.equalsIgnoreCase("h4") || nodeName.equalsIgnoreCase("h5")
					|| nodeName.equalsIgnoreCase("h6") || "heading".equals(tagElement.getAttribute("role"))) {
				if (!beginFound) {
					for (String contactText : contactTexts) {
						Pattern patterAutocomplete = Pattern.compile(contactText, Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
						if (patterAutocomplete.matcher(tagElement.getTextContent()).find()) {
							beginElement = tagElement;
							elements.add((Element) tagElement.cloneNode(false));
							beginFound = true;
						}
					}
				} else if (!endFound) {
					if ("heading".equals(beginElement.getAttribute("role")) && "heading".equals(tagElement.getAttribute("role"))) {
						try {
							Integer beginAriaLevel = Integer.parseInt(beginElement.getAttribute("aria-level"));
							Integer currentAriaLevel = Integer.parseInt(tagElement.getAttribute("aria-level"));
						} catch (Exception e) {
							Logger.putLog("Error al comprobar los aria level", AccesibilityDeclarationCheckUtils.class, Logger.LOG_LEVEL_ERROR);
						}
					} else if (nodeName.equalsIgnoreCase("h1") || nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h3") || nodeName.equalsIgnoreCase("h4")
							|| nodeName.equalsIgnoreCase("h5") || nodeName.equalsIgnoreCase("h6")) {
						final String tagName = beginElement.getTagName().toLowerCase();
						switch (tagName) {
						case "h1":
							...
							break;
						case "h2":
							if (nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h1")) {
								endFound = true;
							}
							break;
						case "h3":
							if (nodeName.equalsIgnoreCase("h3") || nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h1")) {
								endFound = true;
							}
							break;
						case "h4":
							if (nodeName.equalsIgnoreCase("h4") || nodeName.equalsIgnoreCase("h3") || nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h2")) {
								endFound = true;
							}
							break;
						case "h5":
							if (nodeName.equalsIgnoreCase("h5") || nodeName.equalsIgnoreCase("h4") || nodeName.equalsIgnoreCase("h3") || nodeName.equalsIgnoreCase("h2")
									|| nodeName.equalsIgnoreCase("h2")) {
								endFound = true;
							}
							break;
						case "h6":
							if (nodeName.equalsIgnoreCase("h6") || nodeName.equalsIgnoreCase("h5") || nodeName.equalsIgnoreCase("h4") || nodeName.equalsIgnoreCase("h3")
									|| nodeName.equalsIgnoreCase("h2") || nodeName.equalsIgnoreCase("h2")) {
								endFound = true;
							}
							break;
						default:
						...
						if (!Arrays.asList(CONTENTTAGS).contains(nodeName.toLowerCase())) {
							elements.add((Element) tagElement.cloneNode(true));
						}
					}
				}
