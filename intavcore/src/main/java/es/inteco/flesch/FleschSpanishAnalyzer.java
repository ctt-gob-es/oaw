...
public class FleschSpanishAnalyzer implements FleschAnalyzer {
    ...
    public int countSyllables(String text) {
        String[] words = text.split("[^a-zA-ZáéíóúÁÉÍÓÚñÑ]");
        List<String> consonants = Arrays.asList("b", "c", "d", "f", "g", "h", "j", "k", "l", "m", "n",
                "ñ", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z");
        List<String> vowels = Arrays.asList("a", "e", "i", "o", "u", "á", "é", "í", "ó", "ú", "ü");
        List<String> strongVowels = Arrays.asList("a", "e", "o", "á", "é", "ó");
        List<String> weakVowels = Arrays.asList("i", "u");
            ...
            if (words[i] != null && words[i].trim().length() > 0) {
                ...
                for (int j = 0; j < word.length(); j++) {
                    if (consonants.contains(String.valueOf(word.charAt(j)))
                        ...
                        // System.out.print(String.valueOf(word.charAt(j)) + String.valueOf((word.charAt(j+1))) + String.valueOf((word.charAt(j+2))));
                        count++;
                        j += 2;
                        ...
                        count++;
                        // System.out.print(String.valueOf(word.charAt(j)));
                    } else if (strongVowels.contains(String.valueOf(word.charAt(j)))
                            && j + 1 == word.length()) {
                        ...
                        count++;
                        // System.out.print(String.valueOf(word.charAt(j)) + String.valueOf((word.charAt(j+1))));
                        j++;
