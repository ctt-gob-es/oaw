...
public final class Check_1_1_2_HeadersTest {
	...
	public void evaluateCorrectNestingComplex() throws Exception {
		checkAccessibility
				.setContent("<html><head><title>Lorem</title></head><body><h1>Lorem</h1><p>Ipsum</p><h2>Sic semper</h2><p>Foo</p><h3>Bar</h3><p>Lorem</p><h1>Foo</h1><p>Lorem</p></body></html>");
		final Evaluation evaluation = EvaluatorUtils.evaluateContent(checkAccessibility, "es");
		Assert.assertEquals(0, TestUtils.getNumProblems(evaluation.getProblems(), HEADERS_NESTING_ID));
	}
	...
	public void evaluateExistsH1() throws Exception {
		/*
		 * MET 4.2.2 Title: Se verifica la presencia de un encabezado de primer
		 * nivel, en cualquier posición Subject: <h1> Check: debe existir al
		 * menos 1
		 */
	...
	public void evaluateContentBetweenHeaders() throws Exception {
		...
		/*
		 * MET 4.2.4 Title: Se verifica que no haya dos encabezados seguidos del
		 * mismo nivel (o superior) son contenido entre ellos Subject:
		 * <h1>...</h6> Check: Si se pasa de un nivel Hx a un nivel Hx-y
		 * (y=0..x-1) debe haber contenido entre ambos
		 */
	...
	public void evaluateHeadersNesting() throws Exception {
		...
		/*
		 * MET 4.2.5 Title: Se verifica que, a partir del primer encabezado del
		 * documento, e independientemente de cuál sea ese nivel, no se producen
		 * saltos en los niveles de los encabezados sucesivos Subject:
		 * <h1>...</h6> Check: No puede pasarse de un nivel Hx a un nivel Hx+y
		 * (y>=2) Sólo se puede pasarde un nivel Hx a un Hx+y donde
		 * y=(1-x)...(1)
		 */
		...
		checkAccessibility
				.setContent("<html><body><h1>foo</h1><h3>Loren</h3><p>Some content<p><h3>foo</h3><h5>Ipsum</h5><p>Some content<p><h2>Foo</h2><p>bar</p><h6>Ipsum</h6><p>Some content<p></body></html>");
		evaluation = EvaluatorUtils.evaluateContent(checkAccessibility, "es");
	...
	public void evaluateAdjacentSameWAIHeadings() throws Exception {
		checkAccessibility
				.setContent("<html><body><span role=\"heading\" aria-level=\"1\">foo</span><p>Some content</p><span role=\"heading\" aria-level=\"2\">Foo bar</span><p>Some content</p></body></html>");
		Evaluation evaluation = EvaluatorUtils.evaluateContent(checkAccessibility, "es");
		...
		checkAccessibility.setContent(
				"<html><body><span role=\"heading\" aria-level=\"1\">foo</span><p>Some content</p><span role=\"heading\" aria-level=\"2\">Foo bar</span><span role=\"heading\" aria-level=\"2\">Foo bar</span><p>Some content</p></body></html>");
		evaluation = EvaluatorUtils.evaluateContent(checkAccessibility, "es");
		Assert.assertEquals(1, TestUtils.getNumProblems(evaluation.getProblems(), 470));
