...
/**
 * Clase para proporcionar el servicio mail mediante SIM - Plataforma de
 * Mensajería
 */
public class MailSimProvider implements MailProvider {
	...
	public void sendMail() {
		...
		try {
			...
			Logger.putLog(
					String.format("Sending mail from MailSimProvider using %s",
							pmgr.getValue(MAIL_PROPERTIES, "sim.mailservice.wsdl.url")),
					MailSimProvider.class, Logger.LOG_LEVEL_WARNING);
...
//			}
			...
			if (!"1000".equals(respuestaStatus.getStatusCode())) {
				Logger.putLog(String.format("Error SIM response code: %s, text: %s, details: %s",
						respuestaStatus.getStatusCode(), respuestaStatus.getStatusText(), respuestaStatus.getDetails()),
						MailSimProvider.class, Logger.LOG_LEVEL_ERROR);
			}
		} catch (MalformedURLException e) {
			Logger.putLog(
					String.format("Invalid SIM WSDL URL value of %s",
							pmgr.getValue(MAIL_PROPERTIES, "sim.mailservice.wsdl.url")),
					MailSimProvider.class, Logger.LOG_LEVEL_ERROR);
		}
	}
	...
	public void setFromAddress(final String fromAddress) {
		Logger.putLog(
				"Trying to set fromAddress for SIM Mail Service. This value is configured in the SIM Mail Service",
				MailSimProvider.class, Logger.LOG_LEVEL_INFO);
	}

	@Override
	public void setFromName(final String fromName) {
		Logger.putLog("Trying to set fromName for SIM Mail Service. This value is configured in the SIM Mail Service",
				MailSimProvider.class, Logger.LOG_LEVEL_INFO);
	}
