...
public final class RastreoDAO {
	...
	/**
	 * Actualiza el estado de todos los observatorios que están activos
	 * (OBS_LAUNCHED_STATUS) a finalizados con error (OBS_ERROR_STATUS).
	 *
	 * @param connection
	...
	public static void stopRunningObservatories(final Connection connection) throws SQLException {
		try (PreparedStatement pst = connection
				.prepareStatement("UPDATE observatorios_realizados SET estado = ? WHERE estado = ?")) {
	...
	/**
	 * Actualiza el estado de todos los rastreos que están activos (LAUNCHED) a
	 * parados (STOPPED).
	 *
	 * @param c
	 *            conexión (Connection) a la base de datos
	...
	/**
	 * Obtiene las urls (una si es crawling aleatorio o hasta 17 separadas por ';')
	 * de una semilla.
	 *
	 * @param conn
	 *            conexión (Connection) a la base de datos
	 ...
	 * @param type
	 *            tipo de lista (semilla, rastreable, no rastreable u observatorio)
	 *            a la que pertenece la semilla
	 * @return una cadena que contiene la url inicial si es un crawling aleatorio o
	 *         hasta 17 urls separadas por el token ';'
	 * @throws SQLException
	 */
	public static String getList(final Connection conn, final Long idCrawling, final int type) throws SQLException {
		...
		try (PreparedStatement pst = conn.prepareStatement(query)) {
			...
			Logger.putLog("Error al recuperar la semilla del rastreo con id " + idCrawling, RastreoDAO.class,
	 ...
	 *            conexión (Connection) a la base de datos
	 * @param idRastreo
	 *            identificador (long) del rastreo
	...
	public static int recuperarCartuchoPorRastreo(final Connection c, final long idRastreo) throws SQLException {
		try (PreparedStatement pst = c
				.prepareStatement("SELECT id_cartucho FROM cartucho_rastreo WHERE id_rastreo = ?")) {
	 ...
	 *            conexión (Connection) a la base de datos
	 * @param idGuideline
	 *            identificador (long) de la normativa
	...
	public static String recuperarFicheroNorma(final Connection c, final long idGuideline) throws SQLException {
		try (PreparedStatement pes = c
				.prepareStatement("SELECT des_guideline FROM tguidelines WHERE cod_guideline = ?")) {
	 ...
	 *            conexión (Connection) a la base de datos
	 * @param idCartucho
	 *            identificador (long) del cartucho
	...
	public static ExtraInfo getExtraInfo(final Connection c, final Long idRastreoRealizado) throws SQLException {
		try (PreparedStatement pes = c.prepareStatement(
				"SELECT rr.id_obs_realizado, l.nombre, l.lista FROM rastreos_realizados rr JOIN lista l ON rr.id_lista=l.id_lista WHERE id= ?")) {
