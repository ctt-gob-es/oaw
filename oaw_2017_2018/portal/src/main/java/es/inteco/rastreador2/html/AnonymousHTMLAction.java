...
public class AnonymousHTMLAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (requestType.contains(Constants.SEGMENT_RESULTS_1) || requestType.contains(Constants.SEGMENT_RESULTS_3)
                || requestType.contains(Constants.SEGMENT_RESULTS_4) || requestType.contains(Constants.SEGMENT_RESULTS_5)) {
        ...
        String conclusionPath = pmgr.getValue(CRAWLER_PROPERTIES, "path.conclusion.xml") + request.getParameter(Constants.ID_OBSERVATORIO) + File.separator + request.getParameter(Constants.ID) + File.separator + pmgr.getValue(CRAWLER_PROPERTIES, "name.conclusion.xml");
        ...
        if (requestType.equals(Constants.INTRODUCTION) || requestType.equals(Constants.OBJECTIVE) || requestType.equals(Constants.METHODOLOGY)
                || requestType.equals(Constants.METHODOLOGY_SUB1) || requestType.equals(Constants.METHODOLOGY_SUB2)
                || requestType.equals(Constants.METHODOLOGY_SUB3) || requestType.equals(Constants.METHODOLOGY_SUB4)) {
    ...
    private ActionForward createGlobalResults4(HttpServletRequest request, ActionMapping mapping, String noDataMess, java.util.List<ObservatoryEvaluationForm> pageExecutionList, String filePath) throws Exception {
        ...
        ResultadosAnonimosObservatorioIntavUtils.getMidsComparationByVerificationLevelGraphic(request, Constants.OBS_PRIORITY_1, title, file, noDataMess, pageExecutionList, pmgr.getValue(CRAWLER_PROPERTIES, "chart.evolution.mp.green.color"), true);
        ...
        ResultadosAnonimosObservatorioIntavUtils.getMidsComparationByVerificationLevelGraphic(request, Constants.OBS_PRIORITY_2, title, file, noDataMess, pageExecutionList, pmgr.getValue(CRAWLER_PROPERTIES, "chart.evolution.mp.green.color"), true);
