...
public final class BasicServiceUtils {
	...
	public static long saveRequestData(final BasicServiceForm basicServiceForm, final String status) {
		try (Connection conn = DataBaseManager.getConnection()) {
			...
			Logger.putLog("Excepción: Error al insertar los datos del servicio básico,", BasicServiceThread.class,
	...
	public static void updateRequestStatus(final BasicServiceForm basicServiceForm, final String status) {
		try (Connection conn = DataBaseManager.getConnection()) {
			...
			Logger.putLog("Excepción: Error al modificar los datos del servicio básico,", BasicServiceThread.class,
					Logger.LOG_LEVEL_ERROR, e);
		}
	}
	...
	public static List<BasicServiceForm> getBasicServiceRequestByStatus(final String status) {
		try (Connection conn = DataBaseManager.getConnection()) {
			...
			Logger.putLog("Excepción: Error al recuperar las peticiones al servicio básico con el estado " + status,
					BasicServiceThread.class, Logger.LOG_LEVEL_ERROR, e);
		}
		return null;
	}

	public static BasicServiceForm getBasicServiceForm(final BasicServiceForm basicServiceForm,
			final HttpServletRequest request) {
		Logger.putLog("getBasicServiceForm " + basicServiceForm.toString(), BasicServiceUtils.class,
		...
		if (urlParameter != null) {
			...
			try {
				...
				Logger.putLog("No se puede decodificar la url como ISO-8859-1", CheckHistoricoAction.class,
						Logger.LOG_LEVEL_WARNING, e);
				try {
					url = URLDecoder.decode(urlParameter, "UTF-8");
				} catch (UnsupportedEncodingException e1) {
					url = "";
					Logger.putLog("No se puede decodificar la url como UTF-8", CheckHistoricoAction.class,
							Logger.LOG_LEVEL_WARNING, e);
				}
			}
		...
		if (request.getParameter("informe-nobroken") != null
				&& Boolean.parseBoolean(request.getParameter("informe-nobroken"))) {
		...
		if (StringUtils.isNotEmpty(request.getParameter(Constants.PARAM_CONTENT))) {
			try {
				basicServiceForm
						.setContent(new String(request.getParameter(Constants.PARAM_CONTENT).getBytes("ISO-8859-1")));
			} catch (UnsupportedEncodingException e) {
				Logger.putLog("No se puede codificar el contenido como ISO-8859-1", BasicServiceUtils.class,
						Logger.LOG_LEVEL_WARNING, e);
				try {
					basicServiceForm
							.setContent(new String(request.getParameter(Constants.PARAM_CONTENT).getBytes("UTF-8")));
				} catch (UnsupportedEncodingException e1) {
					Logger.putLog("No se puede codificar el contenido como UTF-8", BasicServiceUtils.class,
							Logger.LOG_LEVEL_WARNING, e);
	...
	public static ActionErrors validateReport(final BasicServiceForm basicServiceForm) {
		...
		if (!report.equalsIgnoreCase(Constants.REPORT_UNE) && !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY)
				&& !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_1_NOBROKEN)
				&& !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_2)
				&& !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_2_NOBROKEN)
				&& !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_3)
				&& !report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_3_NOBROKEN)) {
			errors.add(Globals.ERROR_KEY, new ActionMessage("basic.service.report.not.valid",
					Constants.REPORT_OBSERVATORY, Constants.REPORT_UNE));
		}
	...
	public static String checkIDN(final String url) {
		if (url != null && !url.isEmpty()) {
			try {
				...
				Logger.putLog("Error al verificar el dominio internacionalizado", BasicServiceUtils.class,
						Logger.LOG_LEVEL_WARNING, e);
				return url;
			}
		} else {
	...
	public static long getGuideline(final String report) {
		final PropertiesManager pmgr = new PropertiesManager();
		if (report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY)
				|| report.equalsIgnoreCase(Constants.REPORT_OBSERVATORY_FILE)
				|| report.equals(Constants.REPORT_OBSERVATORY_1_NOBROKEN)) {
		...
		} else if (report.equals(Constants.REPORT_OBSERVATORY_2)
				|| report.equals(Constants.REPORT_OBSERVATORY_2_NOBROKEN)) {
			return 7L;
		} else if (report.equals(Constants.REPORT_OBSERVATORY_3)
				|| report.equals(Constants.REPORT_OBSERVATORY_3_NOBROKEN)) {
			return 8L;
		} else {
			return -1;
		}
	...
	public static void scheduleBasicServiceJob(final BasicServiceForm basicServiceForm) throws Exception {
		Logger.putLog("Programando el rastreo con la fecha: " + basicServiceForm.getSchedulingDate(), CrawlerWS.class,
				Logger.LOG_LEVEL_INFO);
		final SchedulerFactory schedulerFactory = new StdSchedulerFactory();
		final Scheduler scheduler = schedulerFactory.getScheduler();
		scheduler.start();

		final JobDetail jobDetail = new JobDetail("CrawlerWSJob_" + System.currentTimeMillis(), "CrawlerWSJob",
				CrawlerWSJob.class);

		if (basicServiceForm.getId() == 0) {
			basicServiceForm.setId(BasicServiceUtils.saveRequestData(basicServiceForm,
					es.inteco.common.Constants.BASIC_SERVICE_STATUS_SCHEDULED));
		}
		...
		final Trigger trigger = new SimpleTrigger("CrawlerWSTrigger", "CrawlerWSGroup",
				basicServiceForm.getSchedulingDate());
	...
	public static String getTitleFromContent(final String content) {
		...
		if (matcher.find()) {
			if (matcher.group(1).length() <= Integer
					.parseInt(pmgr.getValue(CRAWLER_PROPERTIES, "basic.service.title.max.length"))) {
	...
	public static String getTitleDocFromContent(final String content, final boolean isCompleted) {
		...
		try {
			...
			if (matcher.find()) {
				...
				final int titleMaxLength = Integer
						.parseInt(pmgr.getValue(CRAWLER_PROPERTIES, "basic.service.title.doc.max.length"));
			...
			Logger.putLog("Error al acceder al extraer el título del contenido HTML", BasicServiceUtils.class,
					Logger.LOG_LEVEL_WARNING, e);
			return "";
		}
	}
