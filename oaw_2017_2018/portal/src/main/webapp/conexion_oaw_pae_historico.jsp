	...
	class RequestManager {
		...
		private void initProxy() {
			if (StringUtils.isNotEmpty(RUTA_PROXY)) {
				...
				systemProperties.setProperty("sun.net.client.defaultReadTimeout",
						propertiesServices.getMessage("servicio.accesibilidad.read.timeout", null));
				systemProperties.setProperty("http.proxyHost", RUTA_PROXY);
				systemProperties.setProperty("http.proxyPort",
						propertiesServices.getMessage("servicio.accesibilidad.proxy.port", null));
			}
		}
		...
		public String launchServicioDiagnostico() {
			try {
				...
				if (httpConnection.getResponseCode() != 200) {
					...
					return "Error, no se ha podido procesar su solicitud (c&oacute;digo de error "
							+ httpConnection.getResponseCode()
							+ "). Por favor int&eacute;ntelo de nuevo pasados unos minutos y si el problema persiste informe sobre ello a la direcci&oacute;n de correo observ_accesibilidad@correo.gob.es<br/>";
				} else {
		...
		private String buildPostRequest() {
			...
			try {
				...
				final String postRequest = String.format(
						"content=%s&url=%s&correo=%s&profundidad=%s&amplitud=%s&informe=%s&usuario=%s&inDirectory=%s&registerAnalysis=%s&analysisToDelete=%s&informe-nobroken=%s&urls=%s&type=%s",
						encodedCodigo != null ? encodedCodigo : "", url != null ? url : "", correo, profundidad,
						amplitud, informe, username, directorio, registerAnalysis, analysisToDelete, nobroken, urls,
						type);
				return postRequest;
			} catch (Exception e) {
				return "";
		...
		public String generateCallHistoricoJavaScriptFunction(final String historicoResult) {
			return String.format("<script type='text/javascript'>checkHistoricoResults(JSON.parse('%s'));</script>",
					historicoResult);
		}
		...
		private void validateRequest() {
			...
			if (isCrawlingRequest()) {
				...
				if (urls.isEmpty()) {
				...
				} else if (urls.length() > 120000) {
					errores.add(
							"El m&aacute;ximo de caracteres para el 'Conjunto de URLs' es 8000. Acorte las URLs o introduzca menos URLs");
				} else {
					...
					if ("observatorio-3".equals(informe) && domains.length > 33) {
						errores.add(
								"El n&uacute;mero m&aacute;ximo de URLs a analizar para esta metodolog&iacute;a (versi&oacute;n 2) es de 33");
					} else if (("observatorio-2".equals(informe) || "observatorio-1".equals(informe))
							&& domains.length > 17) {
						errores.add(
								"El n&uacute;mero m&aacute;ximo de URLs a analizar para esta metodolog&iacute;a  es de 17");
					}
				}
				...
				if (isEvolutivoHistorico()) {
					...
					if ("observatorio-3".equals(informe) && domains.length > 33) {
						errores.add(
								"El evolutivo para an&aacute;lisis de tipo 'Conjunto de URLs' para esta metodolog&iacute;a (versi&oacute;n 2) requiere 33 p&aacute;ginas");
					} else if (("observatorio-2".equals(informe) || "observatorio-1".equals(informe))
							&& domains.length != 17) {
						errores.add(
								"El evolutivo para an&aacute;lisis de tipo 'Conjunto de URLs' requiere 17 p&aacute;ginas");
					} else {
						try {
							...
							for (String domain : domains) {
								...
								if (!host.equalsIgnoreCase(pattern)) {
									errores.add(
											"El evolutivo para an&aacute;lisis de tipo 'Conjunto de URLs' solo est&aacute; permitido para p&aacute;ginas del mismo 'host' (exactamente mismo dominio) "
													+ domain + " | " + pattern);
									break;
								}
							}
							...
							errores.add(
									"Error al comprobar el dominio de un an&aacute;lisis de tipo 'Conjunto de URLs'");
						}
					}
				}
			} else {
...
#cajaformularios form fieldset, #cajaformularios div.mensaje {
...
#cajaformularios {
	...
	padding: 1% 4%;
	background: #e5e5e5;
	background: -moz-linear-gradient(top, #ffffff 0%, #dbdbdb 100%, #d1d1d1 100%,
		#e5e5e5 100%);
	background: -webkit-linear-gradient(top, #ffffff 0%, #dbdbdb 100%, #d1d1d1 100%,
		#e5e5e5 100%);
	background: linear-gradient(to bottom, #ffffff 0%, #dbdbdb 100%, #d1d1d1 100%,
		#e5e5e5 100%);
	...
	<div id="cajaformularios">
		...
		<form method="POST" enctype="multipart/form-data">
			<input type="hidden" name="confirm" value="true"> <input
				type="hidden" name="url" value="<%=requestManager.url%>"> <input
				type="hidden" name="urls" value="<%=requestManager.urls%>">
			<input type="hidden" name="profundidad"
				value="<%=requestManager.profundidad%>"> <input
				type="hidden" name="amplitud" value="<%=requestManager.amplitud%>">
			<input type="hidden" name="correo" value="<%=requestManager.correo%>">
			<input type="hidden" name="informe"
				value="<%=requestManager.informe%>"> <input type="hidden"
				name="informe-nobroken"
				value="<%=requestManager.nobroken != null ? requestManager.nobroken : ""%>">
			<input type="hidden" name="registerAnalysis"
				value="<%=requestManager.registerAnalysis%>"> <input
				type="hidden" name="type" value="<%=requestManager.type%>">
