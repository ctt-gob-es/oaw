...
<inteco:sesion action="ifConfigAdmin">
	...
	<bean:define id="grRegenerate" ><%= Constants.OBSERVATORY_GRAPHIC_SEGMENT_I %></bean:define>
						...
						<div id="cajaformularios">
							...
							<logic:equal name="<%= Constants.OBSERVATORY_RESULTS %>" value="<%= Constants.SI %>">
								<logic:notEmpty name="<%= Constants.OBSERVATORY_LENOX_GRAPHIC_GLOBAL_PERCENTAGE_TERMS %>">
									<h3><bean:message key="resultados.anonimos.lenox.porcentaje.prioridad.terminos.s1.title" /></h3>
									...
									<div class="divCenter"><img src="<%=request.getContextPath()%>/secure/GraficasObservatorio.do?<%= Constants.TYPE_OBSERVATORY %>=<%= observatoryType %>&amp;<%= Constants.ID %>=<%= id %>&amp;<%= Constants.ID_OBSERVATORIO %>=<%= id_observatorio %>&amp;<%= Constants.GRAPHIC %>=<%= Constants.OBSERVATORY_LENOX_GRAPHIC_GLOBAL_PERCENTAGE_SEG1 %>"></img></div>
								</logic:notEmpty>
