...
public class CheckHistoricoService {
     ...
     */
    public final boolean isAnalysisOfUrl(final String idAnalysis, final String url, BasicServiceAnalysisType type) {
        final List<BasicServiceResultado> historicoResultados = getHistoricoResultados(url, type);
    ...
    public Map<Date, List<ObservatoryEvaluationForm>> getHistoricoResultadosOfBasicService(final BasicServiceForm basicServiceForm) {
        ...
        if (basicServiceForm.isRegisterAnalysis()) {
            final List<BasicServiceResultado> historicoResultados = getHistoricoResultados(basicServiceForm.getDomain(), basicServiceForm.getAnalysisType());
     ...
     */
    public List<BasicServiceResultado> getHistoricoResultados(final String url, final BasicServiceAnalysisType type) {
        if (url != null && url.trim().length() > 0) {
            if (BasicServiceAnalysisType.URL == type) {
                return DiagnosisDAO.getHistoricoResultados(url, type);
            } else if (BasicServiceAnalysisType.LISTA_URLS == type) {
                try {
                    final String host = new URL(url.split("\r\n|\n")[0]).getHost();
                    final String pattern = String.format("(https?://%s.*){17,17}", host);
                    return DiagnosisDAO.getHistoricoResultados(pattern, type);
