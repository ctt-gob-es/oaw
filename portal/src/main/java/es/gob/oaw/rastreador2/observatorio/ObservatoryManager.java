...
public class ObservatoryManager {
	...
	/**
	 * Devuelve los análisis de las páginas correspondientes a la ejecución de un
	 * rastreo con la metodología del observatorio.
	 *
	 * @param idObservatoryExecution identificador correspondiente a una ejecución
	 *                               (iteración) de un observatorio
	 * @param evaluationIds          identificadores de análisis de página que se
	 *                               quieren evaluar
	 * @return una lista con las evaluaciones de las páginas correspondientes a la
	 *         ejecución indicada
	 */
	public List<ObservatoryEvaluationForm> getObservatoryEvaluationsFromObservatoryExecution(
			final long idObservatoryExecution, final List<Long> evaluationIds) {
			...
			for (Long id : evaluationIds) {
				try {
					...
					final ObservatoryEvaluationForm evaluationForm = EvaluatorUtils
							.generateObservatoryEvaluationForm(evaluation, methodology, true, pointWarning);
	...
	public RankingInfo calculatePreviousRanking(final Long idObservatoryExecution, final SemillaForm currentSeed) {
		try (Connection c = DataBaseManager.getConnection()) {
			final Long previousObservatoryExecution = ObservatorioDAO.getPreviousObservatoryExecution(c,
	 ...
	 *
	 * @param idObservatoryExecution el identificador de una iteracción de un
	 *                               observatorio
	 * @param currentSeed            la semilla de la que hay que calcular su
	 *                               ranking en la iteracción
	 ...
	 *
	 * @param idRastreo              id del rastreo del que se quiere obtener el
	 *                               rastreo previo
	 * @param idObservatoryExecution id del observatorio al que pertenece el
	 *                               rastreo.
	 * @return el id del rastreo o 0 si no existe
	 */
	public long getPreviousIdRastreoRealizadoFromIdRastreoAndIdObservatoryExecution(final Long idRastreo,
			final long idObservatoryExecution) {
		try (Connection c = DataBaseManager.getConnection()) {
			return RastreoDAO.getIdRastreoRealizadoFromIdRastreoAndIdObservatoryExecution(c, idRastreo,
					idObservatoryExecution);
		} catch (Exception e) {
			Logger.putLog("Exception: ", ExportAction.class, Logger.LOG_LEVEL_ERROR, e);
		}
