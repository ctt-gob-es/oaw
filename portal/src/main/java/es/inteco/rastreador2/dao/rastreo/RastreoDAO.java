...
public final class RastreoDAO {
    ...
    public static void deleteAnalyse(Connection connection, List<Long> idsRastreosRealizados) throws SQLException {
        if (idsRastreosRealizados != null && !idsRastreosRealizados.isEmpty()) {
            ...
            if (connection.getCatalog().contains("intav")) {
                try (PreparedStatement ps = connection.prepareStatement("DELETE FROM tanalisis WHERE cod_rastreo IN " + executionIdStrList)) {
    ...
    public static DatosCartuchoRastreoForm cargarDatosCartuchoRastreo(Connection c, String nombreRastreo) throws SQLException {
        ...
        final String sq = "SELECT r.*, lg.*, c.id_cartucho, c.nombre as nombre_cartucho, c.numrastreos, ll.acronimo " +
    ...
    public static InsertarRastreoForm cargarRastreo(Connection c, int idRastreo, InsertarRastreoForm rastreo) throws SQLException {
        final PropertiesManager pmgr = new PropertiesManager();
        try (PreparedStatement ps = c.prepareStatement("SELECT r.*, l.nombre AS nombreSemilla, l1.lista AS listaRastreable, " +
    ...
    public static RastreoEjecutadoForm cargarRastreoEjecutado(Connection c, long idExecution) throws SQLException {
        ...
        try (PreparedStatement ps = c.prepareStatement("SELECT r.*, rr.* FROM rastreo r " +
                "JOIN rastreos_realizados rr ON (r.id_rastreo = rr.id_rastreo) " +
                "WHERE rr.id = ? ")) {
    ...
    public static int getNumActiveCrawlings(Connection conn, int cartridge) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement("SELECT COUNT(*) FROM rastreo r " +
                "JOIN cartucho_rastreo cr ON (r.id_rastreo = cr.id_rastreo)" +
    ...
    public static Long addFulfilledCrawling(Connection conn, DatosCartuchoRastreoForm dcrForm,
        ...
        try (PreparedStatement pst = conn.prepareStatement("INSERT INTO rastreos_realizados (id_rastreo, fecha, id_usuario, id_cartucho, id_obs_realizado, id_lista) VALUES (?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    public static List<FulFilledCrawling> getOldCrawlings(Connection conn, int numDays) throws Exception {
        final List<FulFilledCrawling> crawlings = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM rastreos_realizados rr " +
    ...
    public static List<Long> getCrawlerCategoryIds(Connection connR, long idObservatorio, long idCategoria) throws Exception {
        ...
        try (PreparedStatement ps = connR.prepareStatement("SELECT id_rastreo FROM rastreo r " +
