...
public final class RastreoDAO {
    ...
    public static String cargarFechaRastreo(Connection c, String nombreRastreo) throws SQLException {
        ...
        String f = "";
        try {
            ps = c.prepareStatement("SELECT fecha_lanzado FROM rastreo WHERE nombre_rastreo = ?");
            ps.setString(1, nombreRastreo);
            rs = ps.executeQuery();
            while (rs.next()) {
                try {
    ...
    public static String recuperarNombreRastreo(Connection c, Long idCrawling) throws SQLException {
        ...
        ResultSet res = null;
        try {
            pes = c.prepareStatement("SELECT nombre_rastreo FROM rastreo WHERE id_rastreo = ?;");
