 		...
 		try (PreparedStatement ps = c.prepareStatement(query)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) { //iterando por los id de rastreos_realizados
			 		...
			 		try (PreparedStatement ps2 = c.prepareStatement(query)) {
						...
						try (ResultSet rs2 = ps2.executeQuery()) {
							while (rs2.next()) { //iterando por los cod_analisis de tanalisis
								...
								PreparedStatement deleteAnalisisCSS = c.prepareStatement("DELETE FROM tanalisis_css WHERE cod_analisis = ?");
					...
					PreparedStatement deleteAnalisis = c.prepareStatement("DELETE FROM tanalisis WHERE cod_rastreo = ?");
