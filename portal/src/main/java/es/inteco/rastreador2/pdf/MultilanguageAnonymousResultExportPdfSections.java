...
public final class MultilanguageAnonymousResultExportPdfSections {
    ...
    public static int createIntroductionChapter(HttpServletRequest request, IndexEvents index, Document document, int countSections, int numChapter, Font titleFont) throws DocumentException, IOException {
        ...
        chapter.add(PDFUtils.createParagraphWithDiferentFormatWord(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.1.p1"), boldWords, ConstantsFont.paragraphBoldFont, ConstantsFont.paragraphFont, true));

        PDFUtils.addParagraph(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.1.p2"), ConstantsFont.paragraphFont, chapter);
    ...
    public static int createMethodologyChapter(HttpServletRequest request, IndexEvents index, Document document, int countSections, int numChapter, Font titleFont, List<AnalysisForm> analysisList, long observatoryType, boolean isBasicService) throws Exception {
        ...
        Section section3 = PDFUtils.addSection(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.chapter33.title"), index, ConstantsFont.chapterTitleMPFont2L, chapter, countSections++, 1);
        MultilanguageAnonymousResultExportPdfSection3.createSection33(request, section3);

        Section section31 = PDFUtils.addSection(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.chapter331.title"), index, ConstantsFont.chapterTitleMPFont2L, section3, countSections++, 1);
        MultilanguageAnonymousResultExportPdfSection3.createSection331(request, section31);

        Section section32 = PDFUtils.addSection(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.chapter332.title"), index, ConstantsFont.chapterTitleMPFont2L, section3, countSections++, 1);
        MultilanguageAnonymousResultExportPdfSection3.createSection332(request, section32);

        Section section33 = PDFUtils.addSection(CrawlerUtils.getResources(request).getMessage("ob.resAnon.multilanguage.report.chapter333.title"), index, ConstantsFont.chapterTitleMPFont2L, section3, countSections++, 1);
