...
public class SeedCategoriesAction extends Action {
	...
	private ActionForward deleteCategory(ActionMapping mapping, HttpServletRequest request) throws Exception {
		...
		ActionUtils.setSuccesActionAttributes(request, "mensaje.exito.categoria.semilla.eliminada",
				"volver.listado.categorias.semilla");
		return mapping.findForward(Constants.EXITO);
	}

	private ActionForward deleteCategoryConfirmation(ActionMapping mapping, HttpServletRequest request)
			throws Exception {
	...
	private ActionForward addSeedCategory(ActionMapping mapping, ActionForm form, HttpServletRequest request)
			throws Exception {
			...
			if (errors.isEmpty()) {
				if (categoriaForm.getFileSeeds() == null
					...
					try (Connection c = DataBaseManager.getConnection()) {
						...
						String mensaje = getResources(request).getMessage(getLocale(request),
								"mensaje.exito.categoria.semilla.creada", categoriaForm.getName());
						...
						if (categoriaForm.getFileSeeds().getFileData().length > 0) {
							try {
								List<SemillaForm> seeds = SeedUtils
										.getSeedsFromFile(categoriaForm.getFileSeeds().getInputStream(), false);
				...
				} else if (categoriaForm.getFileSeeds().getFileSize() > Integer
						.parseInt(pmgr.getValue(CRAWLER_PROPERTIES, "xml.file.max.size"))) {
	...
	private ActionForward updateSeedCategory(ActionMapping mapping, ActionForm form, HttpServletRequest request)
			throws Exception {
		if (!isCancelled(request)) {
			...
			try (Connection c = DataBaseManager.getConnection()) {
				...
				String mensaje = getResources(request).getMessage(getLocale(request),
						"mensaje.exito.categoria.semilla.editada", categoriaForm.getName());
				...
				if (categoriaForm.getFileSeeds().getFileData().length > 0) {
					try {
						...
						final List<SemillaForm> seeds = SeedUtils
								.getSeedsFromFile(categoriaForm.getFileSeeds().getInputStream(), false);
						// Semillas de la categoria
						final List<SemillaForm> oldSeeds = SemillaDAO.getSeedsByCategory(c,
								Long.parseLong(categoriaForm.getId()), Constants.NO_PAGINACION, new SemillaForm());
	...
	private static void compareSeeds(final Connection c, final String idCategory, final List<SemillaForm> newSeeds,
			final List<SemillaForm> oldList) throws Exception {
	...
	private static void insertSeedsFromXml(final Connection c, final List<SemillaForm> insertSeeds,
			final String idCategory, final List<ObservatorioForm> observatoryFormList) throws Exception {
			...
			for (SemillaForm semillaForm : insertSeeds) {
				for (ObservatorioForm observatorioForm : observatoryFormList) {
					if (observatorioForm.getCategoria() != null
							&& Arrays.asList(observatorioForm.getCategoria()).contains(idCategory)) {
						...
						insertarRastreoForm.setId_semilla(
								SemillaDAO.getIdList(c, semillaForm.getNombre(), Long.parseLong(idCategory)));
	...
	private static void updateSeedsFromXml(final Connection c, final List<SemillaForm> repitSeeds,
			final List<ObservatorioForm> observatoryFormList) throws Exception {
		if (repitSeeds != null && !repitSeeds.isEmpty()) {
			SemillaDAO.updateCategorySeeds(c, repitSeeds);
			for (SemillaForm semillaForm : repitSeeds) {
				for (ObservatorioForm observatorioForm : observatoryFormList) {
					final Long crawlerId = RastreoDAO.getCrawlerFromSeedAndObservatory(c, semillaForm.getId(),
							observatorioForm.getId());
					RastreoDAO.updateCrawlerName(c, observatorioForm.getNombre() + "-" + semillaForm.getNombre(),
							crawlerId);
				}
			}
		}
	...
	private ActionForward viewSeedCategory(final ActionMapping mapping, final ActionForm form,
	...
	private ActionForward deleteSeedConfirmation(final ActionMapping mapping, final HttpServletRequest request)
			throws Exception {
	...
	private ActionForward deleteSeed(final ActionMapping mapping, final HttpServletRequest request) throws Exception {
		...
		try (Connection c = DataBaseManager.getConnection()) {
			...
			final String mensaje = getResources(request).getMessage(getLocale(request),
					"mensaje.exito.semilla.borrada");
			final String volver = pmgr.getValue("returnPaths.properties", "volver.editar.categoria.semilla")
					.replace("{0}", idCategory);
			request.setAttribute("mensajeExito", mensaje);
			request.setAttribute("accionVolver", volver);
	...
	private ActionForward newCategorySeed(final ActionMapping mapping, final HttpServletRequest request)
			throws Exception {
		if (isCancelled(request)) {
	...
	private ActionForward editCategorySeed(final ActionMapping mapping, final HttpServletRequest request)
			throws Exception {
		if (isCancelled(request)) {
			return mapping.findForward(Constants.EDIT_SEED_CATEGORY);
		}
	...
	private ActionForward addCategorySeed(final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request) throws Exception {
		...
		if (!isCancelled(request)) {
			try (Connection c = DataBaseManager.getConnection()) {
				semillaForm.setListaUrlsString(semillaForm.getListaUrlsString().replace("\r\n", ";"));
				final Long idSeed = SemillaDAO.insertList(c, 4, semillaForm.getNombre(),
						semillaForm.getListaUrlsString(), semillaForm.getCategoria().getId(), null, null, null, semillaForm.getAcronimo(),
						null);

				final List<ObservatorioForm> observatoryIds = ObservatorioDAO.getObservatoriesFromCategory(c,
						semillaForm.getCategoria().getId());
				for (ObservatorioForm observatorioForm : observatoryIds) {
					if (observatorioForm.getCategoria() != null && Arrays.asList(observatorioForm.getCategoria())
							.contains(semillaForm.getCategoria().getId())) {
						final InsertarRastreoForm insertarRastreoForm = new InsertarRastreoForm();
						ObservatorioDAO.putDataToInsert(insertarRastreoForm, observatorioForm);
						insertarRastreoForm.setCodigo(observatorioForm.getNombre() + "-" + semillaForm.getNombre());
			...
			final PropertiesManager pmgr = new PropertiesManager();
			final String mensaje = getResources(request).getMessage(getLocale(request),
					"mensaje.exito.categoria.semilla.creada.semilla");
			final String volver = pmgr.getValue("returnPaths.properties", "volver.editar.categoria.semilla")
					.replace("{0}", semillaForm.getCategoria().getId());
			request.setAttribute("mensajeExito", mensaje);
			request.setAttribute("accionVolver", volver);
			return mapping.findForward(Constants.EXITO);
		} else {
			...
			forward.setPath(
					forward.getPath() + "&" + Constants.ID_CATEGORIA + "=" + semillaForm.getCategoria().getId());
			forward.setRedirect(true);
			return forward;
		}
	}

	private ActionForward updateCategorySeed(final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request) throws Exception {
		final SemillaForm semillaForm = (SemillaForm) form;
		...
		if (!isCancelled(request)) {
			...
			final PropertiesManager pmgr = new PropertiesManager();
			final String mensaje = getResources(request).getMessage(getLocale(request),
					"mensaje.exito.categoria.semilla.editada.semilla");
			final String volver = pmgr.getValue("returnPaths.properties", "volver.editar.categoria.semilla")
					.replace("{0}", semillaForm.getCategoria().getId());
			request.setAttribute("mensajeExito", mensaje);
			request.setAttribute("accionVolver", volver);
	...
	private ActionForward getCategorySeedsFile(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		try (Connection c = DataBaseManager.getConnection()) {
			final Long idCategory = Long.parseLong(request.getParameter(Constants.ID_CATEGORIA));
			final List<SemillaForm> seeds = SemillaDAO.getSeedsByCategory(c, idCategory, Constants.NO_PAGINACION,
					new SemillaForm());
