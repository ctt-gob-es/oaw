...
public final class DatabaseExportUtils {
	...
	public static Category getCategoryInfo(final MessageResources messageResources, final CategoriaForm categoriaForm, final Observatory observatory) throws Exception {
		...
		final Map<CategoriaForm, Map<String, BigDecimal>> resultDataBySegment = ResultadosAnonimosObservatorioIntavUtils
				.calculateMidPuntuationResultsBySegmentMap(observatory.getIdExecution().toString(), pageExecutionList, categories);
	...
	public static Site getSiteInfo(final MessageResources messagesResources, ObservatorySiteEvaluationForm observatorySiteEvaluationForm, Category category) throws Exception {
		...
		// Puntuación de cada verificación
		final Map<String, BigDecimal> verificationAndScore = ResultadosAnonimosObservatorioIntavUtils.getVerificationResultsByPoint(observatorySiteEvaluationForm.getPages(),
		...
		// Porcentajes de cada modalidad en cada verificación
		final Map<String, BigDecimal> verificationAndModality = ResultadosAnonimosObservatorioIntavUtils.getVerificationResultsByPointAndModality(observatorySiteEvaluationForm.getPages(),
				Constants.OBS_PRIORITY_NONE);
		for (String key : verificationAndModality.keySet()) {
