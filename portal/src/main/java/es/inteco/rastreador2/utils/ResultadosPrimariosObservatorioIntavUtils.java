...
/**
 * The Class ResultadosPrimariosObservatorioIntavUtils.
 */
public final class ResultadosPrimariosObservatorioIntavUtils {
	...
	/**
	 * Instantiates a new resultados primarios observatorio intav utils.
	 */
	 ...
	 */
	public static void getGlobalAccessibilityLevelAllocationSegmentGraphic(final MessageResources messageResources, final List<ObservatoryEvaluationForm> pageExecutionList, final String title,
			final String filePath, final String noDataMess) throws IOException {
		...
		if (!file.exists()) {
			...
			GraphicsUtils.createPieChart(dataSet, title, messageResources.getMessage("observatory.graphic.page.number"), total, filePath, noDataMess,
	...
	/**
	 * Gets the score by page graphic.
	 *
	 ...
	 * @param filePath          the file path
	 * @param noDataMess        the no data mess
	 * @return the score by page graphic
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static void getScoreByPageGraphic(final MessageResources request, final List<ObservatoryEvaluationForm> pageExecutionList, final String title, final String filePath,
			final String noDataMess) throws IOException {
		final File file = new File(filePath);
		// Si no existe la gráfica, la creamos
		if (!file.exists()) {
			// Si son más de 17 ampliamos la x
			if (pageExecutionList.size() > 17) {
				GraphicsUtils.createBarPageByLevelChart(pageExecutionList, title, "", "", filePath, noDataMess, request, 725, y);
			} else {
	...
	/**
	 * Gets the scores by page.
	 *
	 * @param request           the request
	 * @param pageExecutionList the page execution list
	 * @return the scores by page
	 */
	public static Map<String, BigDecimal> getScoresByPage(HttpServletRequest request, List<ObservatoryEvaluationForm> pageExecutionList) {
		...
		for (ObservatoryEvaluationForm evaluationForm : pageExecutionList) {
			result.put(CrawlerUtils.getResources(request).getMessage(CrawlerUtils.getLocale(request), "observatory.graphic.score.by.page.label", ++pageCounter),
					evaluationForm.getScore().setScale(evaluationForm.getScore().scale() - 1));
		}
		return result;
	}
}